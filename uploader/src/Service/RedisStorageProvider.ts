...
export class RedisStorageProvider
  implements StorageProvider, TempStorageProvider
{
  ...
  constructor() {
    ...
    this.redisClient = createClient({
      url:
        REDIS_URL ??
        `redis://${password}${REDIS_HOST}:${REDIS_PORT}/${REDIS_DB_NUMBER || 0}`,
    });
    ...
    this.redisClient
      .connect()
      .catch((err: unknown) => console.error("Redis Client Error", err));
  }

  static isEnabled(): boolean {
    return !!REDIS_URL || (!!REDIS_HOST && !!REDIS_PORT);
  }
  ...
  copyFile(fileId: string, target: TargetDevice): void {
    this.get(fileId)
      ...
      .catch((err: unknown) =>
        console.error("Could not copy file from Redis", err),
      );
  }
}

export const redisStorageProvider = RedisStorageProvider.isEnabled()
